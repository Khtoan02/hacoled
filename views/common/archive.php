<?php
/**
 * Common Blog Archive List View - Premium Luxury Journal Layout (TGDD & Apple Style)
 *
 * @var string $title
 * @var array  $posts
...
<main class="relative bg-[#FAFAFA] pt-28 md:pt-56 pb-24 overflow-hidden min-h-[95vh] bg-tech-grid">
  ...
  <div class="max-w-[1440px] mx-auto px-4 lg:px-8 relative z-10 space-y-10">
    ...
    <nav aria-label="Breadcrumb" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
      ...
      <span class="text-slate-800 font-extrabold"><?php echo esc_html($title); ?></span>
    </nav>
      ...
      <div class="flex items-center gap-3">
        ...
        ?>
          <a href="<?php echo esc_url($cat_link); ?>" 
             class="group flex items-center gap-2 text-xs font-bold px-4 py-2.5 rounded-xl border transition-all duration-300 whitespace-nowrap bg-white text-slate-600 border-slate-250 hover:bg-slate-50 hover:text-slate-900">
            <i class="ph-bold <?php echo $data['icon']; ?> text-slate-400 group-hover:text-accent-red text-sm transition-colors"></i>
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 items-stretch">
              ...
              <div class="md:col-span-7 relative min-h-[240px] md:min-h-[360px] overflow-hidden bg-slate-50">
                ...
                <span class="absolute top-5 left-5 px-3 py-1 bg-accent-red text-white text-[9px] font-black uppercase tracking-widest rounded-md shadow-md">
                  <?php _e('BÀI VIẾT NỔI BẬT', 'hacoled'); ?>
                </span>
              </div>
              ...
              <div class="md:col-span-5 p-6 md:p-8 flex flex-col justify-between space-y-4">
                ...
                <div>
                  <a href="<?php echo esc_url($featured_post['permalink']); ?>" 
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke-width="3.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/></svg>
                  </a>
                </div>
              </div>
        ...
        <div class="space-y-6">
          ...
          <?php else: ?>
            <div class="bg-white border border-slate-200 rounded-3xl p-12 text-center space-y-4 shadow-sm">
              ...
              <h4 class="text-sm font-bold text-slate-800"><?php _e('Chưa có bài viết nào', 'hacoled'); ?></h4>
              <p class="text-slate-500 text-xs max-w-sm mx-auto font-light leading-relaxed">
                <?php _e('Nội dung trong danh sách lưu trữ này đang được cập nhật.', 'hacoled'); ?>
              </p>
            </div>
          <?php endif; ?>
        </div>
